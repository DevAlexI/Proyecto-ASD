...
class Cliente(tk.Frame):
    ...
    def mandarArchivo(self):
        ...
        try:
           ...
           # peso = os.path.getsize(file)
            #BUFFER = 5120
            #progreso = tqdm.tqdm(range(peso), f"[->] Enviando {file}", unit="B", unit_scale=True, unit_divisor=1024)
            ...
            #        if not leerBytes:
            #            print("Se terminó de mandar los datos")
            #            progreso.close()
                       # break
            #        self.sock.sendall(leerBytes)
            #        progreso.refresh()
            #        progreso.update(len(leerBytes))
            #print(f"[+] {file} enviado con exito")
        except:
